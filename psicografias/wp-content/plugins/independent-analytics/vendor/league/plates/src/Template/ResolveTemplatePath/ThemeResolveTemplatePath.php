...
final class ThemeResolveTemplatePath implements ResolveTemplatePath
{
    ...
    public function __invoke(Name $name) : string
    {
        ...
        throw new TemplateNotFound($name->getName(), \array_map(function (array $tup) {
            return $tup[1];
        }, $searchedPaths), \sprintf('The template "%s" was not found in the following themes: %s', $name->getName(), \implode(', ', \array_map(function (array $tup) {
            return \implode(':', $tup);
        }, $searchedPaths))));
    }
}
