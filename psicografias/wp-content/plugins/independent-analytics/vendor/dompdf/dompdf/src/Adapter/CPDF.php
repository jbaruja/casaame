...
namespace IAWP_SCOPED\Dompdf\Adapter;

use IAWP_SCOPED\Dompdf\Canvas;
use IAWP_SCOPED\Dompdf\Dompdf;
use IAWP_SCOPED\Dompdf\Exception;
use IAWP_SCOPED\Dompdf\FontMetrics;
use IAWP_SCOPED\Dompdf\Helpers;
use IAWP_SCOPED\Dompdf\Image\Cache;
use IAWP_SCOPED\FontLib\Exception\FontNotFoundException;
/**
...
class CPDF implements Canvas
{
    ...
    public function __construct($paper = "letter", string $orientation = "portrait", ?Dompdf $dompdf = null)
    {
        ...
        $this->_pdf = new \IAWP_SCOPED\Dompdf\Cpdf($size, \true, $this->_dompdf->getOptions()->getFontCache(), $this->_dompdf->getOptions()->getTempDir());
    ...
    public function select($x, $y, $w, $h, $font, $size, $color = [0, 0, 0], $opts = [])
    {
        ...
        $ft = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_CHOICE;
        $ff = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_CHOICE_COMBO;
    ...
    public function textarea($x, $y, $w, $h, $font, $size, $color = [0, 0, 0])
    {
        ...
        $ft = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_TEXT;
        $ff = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_TEXT_MULTILINE;
    ...
    public function input($x, $y, $w, $h, $type, $font, $size, $color = [0, 0, 0])
    {
        ...
        switch ($type) {
            ...
            case 'password':
                $ft = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_TEXT;
                $ff = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_TEXT_PASSWORD;
                break;
            case 'submit':
                $ft = \IAWP_SCOPED\Dompdf\Cpdf::ACROFORM_FIELD_BUTTON;
                break;
        }
        $pdf->addFormField($ft, \rand(), $x, $this->y($y) - $h, $x + $w, $this->y($y), $ff, $size, $color);
    }
