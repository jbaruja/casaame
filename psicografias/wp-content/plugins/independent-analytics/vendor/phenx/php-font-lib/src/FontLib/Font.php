...
class Font
{
    ...
    public static function load($file)
    {
        ...
        switch ($header) {
            ...
            case "true":
            case "typ1":
                $class = "IAWP_SCOPED\\TrueType\\File";
                break;
            case "OTTO":
                $class = "IAWP_SCOPED\\OpenType\\File";
                break;
            case "wOFF":
                $class = "IAWP_SCOPED\\WOFF\\File";
                break;
            case "ttcf":
                $class = "IAWP_SCOPED\\TrueType\\Collection";
                break;
            // Unknown type or EOT
            default:
                $magicNumber = \file_get_contents($file, \false, null, 34, 2);
                if ($magicNumber === "LP") {
                    $class = "IAWP_SCOPED\\EOT\\File";
                }
        }
        if ($class) {
            $class = "FontLib\\{$class}";
