...
class Hostinger_AutoComplete_Steps {
	...
	public function __construct() {
		...
		add_action( 'save_post_product', [ $this, 'new_product_creation' ], 10, 3 );
		add_action( 'save_post_post', [ $this, 'new_post_creation' ], 10, 3 );
	...
	public function new_post_item_creation( int $post_id, bool $update, string $action ): void {
		...
		if ( ! $update && $session_value === $action ) {
			$this->completed_steps[] = $action;
			Hostinger_Settings::update_setting( 'onboarding_steps', $this->completed_steps );
			unset( $_SESSION[ $action ] );
		}
	...
	public function check_option_change( string $option_name, $old_value, $new_value ): void {
		...
		if ( $session_value === $action && $new_value !== '' && $option_name === Hostinger_Settings::SITE_TITLE_OPTION && $old_value !== $new_value ) {
			$this->completed_steps[] = $action;
			Hostinger_Settings::update_setting( 'onboarding_steps', $this->completed_steps );
			unset( $_SESSION[ $action ] );
		}
